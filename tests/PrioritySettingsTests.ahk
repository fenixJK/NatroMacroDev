TestPrioritySettings() {
	...
	try {
		...
		state := nm_PrioritySettings.Commit(87654321, 12345678)
		AssertEqual(state.Order, 87654321, "Successful save returns candidate")
		AssertEqual(nm_PrioritySettings.Read().Order, state.Order, "Saved order round trips")
		nm_PrioritySettings.Commit(21345678)
		PriorityExpectFailure(ObjBindMethod(nm_PrioritySettings, "Commit", 12345678, state.Order))
		AssertEqual(state.Order, 87654321, "Failed stale save does not publish candidate")
