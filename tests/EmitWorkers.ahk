...
for kind in ["discord", "bee", "priority"] {
	emittedGuiConfig := nm_GuiScripts.Defaults(kind)
	if kind = "discord"
		emittedGuiConfig["webhook"] := 'Unicode Ω "quoted" `` data`nsecond line'
	workers["gui-" kind] := nm_GuiScripts.Build(kind, emittedGuiConfig)
}
